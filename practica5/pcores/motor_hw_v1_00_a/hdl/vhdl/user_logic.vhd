...
--  motor_ctl (0) <= '0'; -- dir
  ...
  signal debd : std_logic;  
  signal debrisingd : std_logic;
  signal derecha : std_logic;
  signal boton_dir : std_logic_vector( 1 downto 0);
...
begin
  ...
  -- 
  ------------------------------------------
rightdeb : debouncer port map (
	 rst => not Bus2IP_Reset,
    clk => Bus2IP_Clk,
    x => boton,
    xDeb => debd,
    xDebFallingEdge => derecha,
    xDebRisingEdge => debrisingd
);
...
process(Bus2IP_Clk, Bus2IP_Reset, derecha)
begin
	if Bus2IP_Reset='1' then
		boton_dir <= "00";
	elsif (Bus2IP_Clk'event and Bus2IP_Clk='1') then
		if derecha='1' then
			if internal_dir = '1' then
				boton_dir <= "01";
			else
				boton_dir <= "10";
			end if;
...
process(boton_dir)
begin
	if boton_dir = "00" then
		dir <= internal_dir;
	else
		if boton_dir = "10" then
			dir <= '1';
		else 
			dir <= '0';
		end if;
